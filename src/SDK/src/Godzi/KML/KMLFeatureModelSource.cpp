...
#include <Godzi/KML/KMLFeatureModelSource>
#include <Godzi/KML/KMLGeometrySymbolizer>
...
namespace
{
    // simple normal-based culling callback
    ...
    /** creates a symbolizer graph for KML features --
        gw-- not sure we actually need this */
    class FactoryKMLSymbolizer : public SymbolizerFactory
    {
    protected:
        osg::ref_ptr<FeatureModelSource> _model;

    public:
        FactoryKMLSymbolizer(FeatureModelSource* model) : _model(model) {}
...
KMLFeatureModelSource::KMLFeatureModelSource( const FeatureModelSourceOptions& options ) : 
...
{
    return new FeatureSymbolizerGraph(new FactoryKMLSymbolizer(this));
}
